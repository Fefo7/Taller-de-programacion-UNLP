...
procedure InformarSociosOrdenDecreciente(a: arbol);
	...
	begin
		...
		writeln ('Numero: ', a^.dato.numero, ' Nombre: ', a^.dato.nombre, ' Edad: ', a^.dato.edad);
		if ((a <> nil) and (a^.HI <> nil)) then InformarSociosOrdenDecreciente (a^.HI);
	end;
...
procedure InformarExistenciaNombreSocio(a:arbol);
	...
	begin
		if(a = nil) then  buscarNombre:= false
			...
			if (a^.dato.nombre =  nombre) then  buscarNombre:=true
			 else
				buscarNombre:=buscarNombre(a^.HD, nombre) or buscarNombre(a^.HD, nombre);		
	end;
...
var
	nombre:cadena15;
begin
	writeln('ingrese el nombre que quiere buscar');
	read(nombre);
	if (buscarNombre(a, nombre)) then
    writeln('El nombre ', nombre, ' fue encontrado.')
  else
    writeln('El nombre ', nombre, ' no fue encontrado.');
end;
...
procedure InformarCantidadSocios (a: arbol;var cant:integer;var suma:integer);
begin
	if (a<> nil) then
	begin
		cant:= cant +1;
		suma:= suma + a^.dato.edad;
		InformarCantidadSocios(a^.HI,cant,suma);
		InformarCantidadSocios(a^.HD,cant,suma);
		 
	end; 
end;
procedure InformarPromedioDeEdad(suma: integer; cant: Integer);
begin
	writeln(suma);
	writeln('el promedio de las edades es de: ', (suma/cant):2:0);
end;
...
Begin
  ...
  InformarCantidadSocios(a,cant,suma);
  writeln('la cantidad de socio es: ', cant); 
  InformarPromedioDeEdad (suma, cant); 
