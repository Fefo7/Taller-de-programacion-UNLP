...
procedure InformarDatosSocioCodeMasChico(a: arbol);
	procedure buscarSocio(a: arbol; var min: integer; var socioAux: socio);
	begin
		if (a <> nil) then
		begin
			...
			begin
			 ...
			 buscarSocio(a^.HI, min,socioAux);  
			 buscarSocio(a^.HD, min,socioAux);  
		end;
	end;
var
	min: integer;
...
begin
	min := 999;
	 buscarSocio(a, min,socioAux);
	writeln('-------------------------------------------------');
	writeln('Socio con el número de socio más chico');
	writeln('Número: ', socioAux.numero);
	writeln('Nombre: ', socioAux.nombre);
	writeln('Edad: ', socioAux.edad);
	writeln('-------------------------------------------------');
end;
...
procedure ExisteSocio(a:arbol);
	...
	begin 
		if (a=nil) then Existe:= false
			...
			if (a^.dato.numero = val) then
				Existe:= true
			else
				Existe:= Existe(a^.HI,val) or Existe(a^.HD,val); {si lo pones separado se pisan los valores}
	end;
var
	valor:integer;
begin
	...
	readln(valor);
	if (Existe(a,valor))  then
		writeln('se encotro al socio')
	else
		writeln('no se encontro el socio');

end;

procedure CantidadSociosComprendidos(a:arbol);
	procedure CalcularSocios(a:arbol; code1,code2: integer;var cant: integer);
	begin
		if(a<> nil) then
		begin
			if(a^.dato.numero >= code1) and (a^.dato.numero <= code2) then
				cant:= cant+1;
			CalcularSocios(a^.HI, code1,code2,cant);
			CalcularSocios(a^.HD, code1,code2,cant);
		end;
	end;
var
	code1,code2:integer;
...
begin
	...
	readln(code1);
	writeln('ingrese el codigo 2 (tiene que ser mayor que el primero): ');
	readln(code2);
	CalcularSocios(a, code1,code2, cant);
	writeln('la cantidad socios que hay entre ', code1, '-', code2, ' es de: ', cant);
end;
...
Begin
  ...
  { ii, iii, iv}
  InformarDatosSocioCodeMasChico(a);
  ExisteSocio(a);
  CantidadSociosComprendidos(a)
